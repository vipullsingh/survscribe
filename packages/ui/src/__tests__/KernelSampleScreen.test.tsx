...
describe("KernelSampleScreen — design kernel end-to-end", () => {
  it("renders the primary-blue button (#1E3A8A, D30)", async () => {
    await render(<KernelSampleScreen />);
  ...
  it("renders the page title in the Plus Jakarta Sans / 20px type-scale row", async () => {
    await render(<KernelSampleScreen />);
  ...
  it("renders a monospace, right-aligned rupee figure with correct Indian grouping", async () => {
    await render(<KernelSampleScreen />);
  ...
  it("consumes required and error affordances on a real form field", async () => {
    await render(<KernelSampleScreen />);
