...
- **Stepped Mobile Registration Flow**:
  ...
  - **Step 2: Professional SLA Credentials & Password**:
    - SLA / IRDAI License Number input (e.g., `SLA-10294`).
...
- **Split-Screen Desktop Layout**:
  ...
  - **Right Registration Card (55% width)**: Comprehensive two-column onboarding form capturing personal, firm, and SLA license data with live format validation.
...
3. **Step 2 Components: SLA Credentials & Security**:
   - `SLALicenseNumberInput` (With format validator: `SLA-[0-9]{4,8}`)
...
| `sla_license_no` | String | Yes | Format: `SLA-[0-9]{4,8}` or alphanumeric | Surveyor license number |
| `sla_category` | Enum | Yes | Fellow / Associate / Licentiate / Trainee | License qualification level |
...
- **Biometric Prompt**: After registration, prompts the user to enable FaceID / TouchID for future instant logins.
